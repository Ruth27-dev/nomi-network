...
@section('layout')
    <div class="content-wrapper" x-data="missionVisionPage">
        @include('admin::shared.header', [
            'title' => __('form.name.mission_vision'),
            'header_name' => __('form.name.mission_vision'),
        ])
        <div class="content-body">
            <div class="content-tab">
                ...
                <div class="content-action-button">
                    ...
                    @can('mission-vision-create')
                        <button class="btn-create" @click="openStoreMissionVisionDialog()">
            ...
            @include('admin::pages.page.mission-vision.table')
        </div>
        @include('admin::pages.page.mission-vision.store')
    </div>
@stop
@section('script')
    <script src="{{ asset('plugin/tinymce/tinymce.min.js') }}" referrerpolicy="origin"></script>
    <script type="module">
        Alpine.data('missionVisionPage', () => ({
            table: new Table("{{ route('admin-page-mission-vision-data') }}"),
            init() {
                this.table.init();
                feather.replace();
            },
            ...
            openStoreMissionVisionDialog(data) {
                this.$dialog('storeMissionVisionDialog').open({
                    data: {
                        ...data,
                        tmp_file: data?.image,
                    },
            ...
            onUpdateStatus(data, status) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-mission-vision-status') }}`,
                                method: 'POST',
                                data: {
                                    id: data.id,
                                    status: status
            ...
            onDelete(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-mission-vision-delete') }}`,
                                method: 'DELETE',
                                data: {
                                    id: data.id
                                }
            ...
            onRestore(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-mission-vision-restore') }}`,
                                method: 'PUT',
                                data: {
                                    id: data.id
                                }
