...
@section('layout')
    <div class="content-wrapper" x-data="reportDocumentPage">
        @include('admin::shared.header', [
            'title' => __('form.name.report_document'),
            'header_name' => __('form.name.report_document'),
        ])
        <div class="content-body">
            ...
            @include('admin::pages.page.report-document.table')
        </div>
        @include('admin::pages.page.report-document.store')
    </div>
@stop
@section('script')
    <script type="module">
        Alpine.data('reportDocumentPage', () => ({
            categories: @json($categories),
            table: new Table("{{ route('admin-page-report-document-data') }}"),
            init() {
                this.table.init();
                feather.replace();
            },
            ...
            openStoreDialog(data) {
                this.$dialog('storeReportDocumentDialog').open({
                    data: {
                        ...data,
            ...
            onUpdateStatus(data, status) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-report-document-status') }}`,
                                method: 'POST',
                                data: {
                                    id: data.id,
                                    status: status
            ...
            onDelete(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-report-document-delete') }}`,
                                method: 'DELETE',
                                data: {
                                    id: data.id
                                }
            ...
            onRestore(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-report-document-restore') }}`,
                                method: 'PUT',
                                data: {
                                    id: data.id
                                }
