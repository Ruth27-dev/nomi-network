<div class="table">
                        ...
                        <div class="flex-auto border-t-[1px] border-b-[1px] border-gray-200 bg-gray-50">
                            <div class="flex h-11">
                                ...
                                <div class="w-15/100 text-sm font-bold text-gray-500 flex items-center">
                                    <span>@lang('table.field.date')</span>
                                </div>
                                <div class="w-15/100 text-sm font-bold text-gray-500 flex items-center">
                                    <span>File</span>
                                </div>
                        ...
                        <div class="flex-auto border-b-[1px] border-gray-200">
                            <div class="flex row-item h-full hover:bg-type_gray">
                                ...
                                <div class="w-20/100 text-gray-500 flex items-center">
                                    <span class="text-sm text-center truncate" x-text="trans(item.category?.title)"></span>
                                </div>
                                <div class="w-15/100 text-gray-500 flex items-center">
                                    <span class="text-sm text-center" x-text="formatDate(item.add_on?.date)"></span>
                                </div>
                                <div class="w-15/100 text-gray-500 flex items-center">
                                    <template x-if="item.file_url">
                                        <a :href="item.file_url" target="_blank" class="text-blue-600 underline text-sm">
                                            Download
                                        </a>
                                    </template>
                                    <template x-if="!item.file_url">
                                ...
                                <div class="w-5/100 text-gray-300 grid place-items-center">
                                    <div x-data="{ open: false, toggle() { this.open = !this.open }, close() { this.open = false } }"
                                        ...
                                        @canany(['report-document-update', 'report-document-delete'])
                                                ...
                                                <template x-if="!item.deleted_at">
                                                    <li>
                                                        <a class="dropdown-item" @click="onDelete(item)">
                                                            ...
                                                            <span class="text-sm text-gray-600 ml-2">@lang('table.option.move_to_trash')</span>
                                                        </a>
                                                    </li>
                                                </template>
                                            @endcan
    ...
    <template x-if="table && table?.empty()">
        @component('admin::components.empty', [
            'name' => __('table.empty.title', ['name' => __('form.title.report_document')]),
            'msg' => __('table.empty.message', ['name' => __('form.title.report_document')]),
        ])
        @endcomponent
    </template>
</div>
