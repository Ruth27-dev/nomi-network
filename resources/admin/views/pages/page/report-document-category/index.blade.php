...
@section('layout')
    <div class="content-wrapper" x-data="reportDocumentCategoryPage">
        @include('admin::shared.header', [
            'title' => __('form.name.report_document_category'),
            'header_name' => __('form.name.report_document_category'),
        ])
        <div class="content-body">
            <div class="content-tab">
                ...
                <div class="content-action-button">
                    ...
                    @can('report-document-category-create')
            ...
            @include('admin::pages.page.report-document-category.table')
        </div>
        @include('admin::pages.page.report-document-category.store')
    </div>
@stop
@section('script')
    <script type="module">
        Alpine.data('reportDocumentCategoryPage', () => ({
            table: new Table("{{ route('admin-page-report-document-category-data') }}"),
            init() {
                this.table.init();
                feather.replace();
            },
            ...
            openStoreDialog(data) {
                this.$dialog('storeReportDocumentCategoryDialog').open({
                    data: data,
                    config: {
                        width: '800px',
                        position: 'right',
            ...
            onUpdateStatus(data, status) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-report-document-category-status') }}`,
                                method: 'POST',
                                data: {
                                    id: data.id,
                                    status: status
            ...
            onDelete(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-report-document-category-delete') }}`,
                                method: 'DELETE',
                                data: {
                                    id: data.id
                                }
            ...
            onRestore(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-report-document-category-restore') }}`,
                                method: 'PUT',
                                data: {
                                    id: data.id
                                }
