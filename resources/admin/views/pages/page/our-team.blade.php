...
@section('layout')
    <div class="form-admin" x-data="contactUsPage">
        @include('admin::shared.header', [
            'title' => __('form.name.our_team'),
            'header_name' => __('form.name.our_team'),
        ])
                        ...
                        <template x-for="(item, index) in dataDetail">
                            <div class="row-2 border-[#d8dce5] border rounded p-3 relative" :key="index"
                                ...
                                <div class="form-row">
                                    ...
                                    <input type="text" x-model="item.name_en" :disabled="form.disabled"
                                        placeholder="@lang('form.body.placeholder.name_en')" autocomplete="off">
                                    <span class="error" x-show="validate?.[`dataDetail.${index}.name_en`]"
                                        x-text="validate?.[`dataDetail.${index}.name_en`]"></span>
                                </div>

                                <!-- name_km -->
                                <div class="form-row">
                                    <label>@lang('form.body.label.name_km') <span>*</span></label>
                                    <input type="text" x-model="item.name_km" :disabled="form.disabled"
                                        placeholder="@lang('form.body.placeholder.name_km')" autocomplete="off">
                                    <span class="error" x-show="validate?.[`dataDetail.${index}.name_km`]"
                                        x-text="validate?.[`dataDetail.${index}.name_km`]"></span>
                                </div>

                                <!-- position_en -->
                                <div class="form-row">
                                    <label>@lang('form.body.label.position_en') <span>*</span></label>
                                    <input type="text" x-model="item.position_en" :disabled="form.disabled"
                                        placeholder="@lang('form.body.placeholder.position_en')" autocomplete="off">
                                    <span class="error" x-show="validate?.[`dataDetail.${index}.position_en`]"
                                        x-text="validate?.[`dataDetail.${index}.position_en`]"></span>
                                </div>

                                <!-- position_km -->
                                <div class="form-row">
                                    <label>@lang('form.body.label.position_km') <span>*</span></label>
                                    <input type="text" x-model="item.position_km" :disabled="form.disabled"
                                        placeholder="@lang('form.body.placeholder.position_km')" autocomplete="off">
                                    <span class="error" x-show="validate?.[`dataDetail.${index}.position_km`]"
                                        x-text="validate?.[`dataDetail.${index}.position_km`]"></span>
                                </div>
                                ...
                                <div class="form-row">
                                    ...
                                    <input type="file" accept="image/*" class="!p-[12px]" :disabled="form.disabled"
                                        @change="onPreviewProfile($event, index)">
                                    <span class="error" x-show="validate?.[`dataDetail.${index}.profile`]"
                                        x-text="validate?.[`dataDetail.${index}.profile`]"></span>

                                    <!-- preview -->
                                    <template x-if="item.profile_url">
                                        <div
                                            class="h-[130px] rounded-md border border-gray-100 overflow-hidden relative grid place-items-center group mt-2">
                                            <img class="w-full h-full object-contain" :src="item.profile_url"
                                                alt="">
                                            <div
                                                ...
                                                <button type="button"
                                                    ...
                                                    @click="onViewProfile(item.profile_url)">
                                                    <span
...
@section('script')
    ...
    <script type="module">
        Alpine.data('contactUsPage', () => ({
            ...
            async init() {
                ...
                if (data) {
                    ...
                    this.dataDetail = (data?.content?.dataDetail || []).map(item => ({
                        ...item,
                        tmp_profile: item.profile || null,
                        profile: null,
                        profile_url: item.profile ? (item.profile.startsWith('http') ? item
                            .profile : this
                            .baseUrl + item.profile) : null,
                    }));
            ...
            onPreviewProfile(event, index) {
                const file = event.target.files[0];
                if (!file) return;
                this.dataDetail[index].profile = file;
                this.dataDetail[index].profile_url = URL.createObjectURL(file);
            },
            ...
            onRemoveProfile(index) {
                this.dataDetail[index].profile = null;
                this.dataDetail[index].profile_url = null;
            },
                ...
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: async (result) => {
                        ...
                        this.dataDetail.forEach((item, index) => {
                            formData.append(`dataDetail[${index}][name_en]`, item
                                .name_en ?? '');
                            formData.append(`dataDetail[${index}][name_km]`, item
                                .name_km ?? '');
                            formData.append(`dataDetail[${index}][position_en]`, item
                                .position_en ?? '');
                            formData.append(`dataDetail[${index}][position_km]`, item
                                .position_km ?? '');
                            ...
                            if (item.profile instanceof File) {
                                formData.append(`dataDetail[${index}][profile]`, item
                                    .profile);
                            }
                            if (item.tmp_profile) {
                                formData.append(`dataDetail[${index}][tmp_profile]`, item
                                    .tmp_profile);
                            }
                        });

                        Axios.post(`{{ route('admin-page-our-team-save') }}`, formData, {
                            headers: {
