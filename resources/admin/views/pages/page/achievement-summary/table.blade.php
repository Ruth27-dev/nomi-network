<div class="table">
        ...
        <div class="table-wrapper">
                    ...
                    <div class="w-full flex gap-3">
                        <div class="flex-auto border-t-[1px] border-b-[1px] border-gray-200 bg-gray-50">
                            <div class="flex h-11">
                                ...
                                <div class="w-10/100 text-sm font-bold text-gray-500 flex items-center">
                                    <span>@lang('table.field.achievement_number')</span>
                                </div>
            ...
            <div class="table-body !w-full !p-0">
                <template x-for="(item,index) in table.data">
                    <div class="w-full flex gap-3 h-[76px]" x-data="{ status: item.status }">
                        <div class="flex-auto border-b-[1px] border-gray-200">
                            <div class="flex row-item h-full hover:bg-type_gray">
                                <div class="w-5/100 grid place-items-center text-gray-500">
                                    <span class="text-sm" x-text="item.iteration ?? index + 1"></span>
                                </div>
                                <div class="w-10/100 text-gray-500 flex items-center">
                                    <div @click="onViewImage(item.image_url)" class="cursor-pointer h-[50px] w-[50px]">
                                        <img x-bind:src="item.image_url" class="h-full w-full rounded-[50px] object-contain"
                                            onerror="(this).src='{{ asset('images/no.jpg') }}'" alt="" />
                                    </div>
                                </div>
                                <div class="w-10/100 text-gray-500 flex items-center">
                                    <span class="text-sm text-center" x-text="item.code ?? '-'"></span>
                                </div>
                                <div class="w-20/100 text-gray-500 flex items-center">
                                    <span class="text-sm text-center truncate" x-text="trans(item.title)"></span>
                                </div>
                                <div class="w-25/100 text-gray-500 flex items-center">
                                    <span class="text-sm text-center truncate" x-text="trans(item.description)"></span>
                                </div>
                                <div class="w-10/100 text-gray-500 flex items-center">
                                    <span class="text-sm text-center" x-text="item.sequence ?? '-'"></span>
                                </div>
                                ...
                                <div class="w-5/100 text-gray-300 grid place-items-center">
                                    <div x-data="{
                                        ...
                                        @canany(['achievement-summary-update', 'achievement-summary-delete'])
                                        ...
                                        <ul x-ref="panel" x-show="open" x-transition.origin.top.right
                                            ...
                                            @can('achievement-summary-update')
                                                <li x-show="!item.deleted_at">
                                                    <a class="dropdown-item" @click="openStoreAchievementSummaryDialog(item)">
                                                        <span
                                            ...
                                            @endcan
                                            @can('achievement-summary-delete')
    ...
    <template x-if="table && table?.empty()">
        @component('admin::components.empty', [
            'name' => __('table.empty.title', ['name' => __('form.title.achievement_summary')]),
            'msg' => __('table.empty.message', ['name' => __('form.title.achievement_summary')]),
        ])
        @endcomponent
    </template>
</div>
