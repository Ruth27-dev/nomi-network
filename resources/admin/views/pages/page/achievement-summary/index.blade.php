...
@section('layout')
    <div class="content-wrapper" x-data="achievementSummaryPage">
        @include('admin::shared.header', [
            'title' => __('form.name.achievement_summary'),
            'header_name' => __('form.name.achievement_summary'),
        ])
        <div class="content-body">
            <div class="content-tab">
                ...
                <div class="content-action-button">
                    ...
                    @can('achievement-summary-create')
                        <button class="btn-create" @click="openStoreAchievementSummaryDialog()">
            ...
            @include('admin::pages.page.achievement-summary.table')
        </div>
        @include('admin::pages.page.achievement-summary.store')
    </div>
@stop
@section('script')
    <script src="{{ asset('plugin/tinymce/tinymce.min.js') }}" referrerpolicy="origin"></script>
    <script type="module">
        Alpine.data('achievementSummaryPage', () => ({
            table: new Table("{{ route('admin-page-achievement-summary-data') }}"),
            init() {
                this.table.init();
                feather.replace();
            },
            ...
            openStoreAchievementSummaryDialog(data) {
                this.$dialog('storeAchievementSummaryDialog').open({
                    data: {
                        ...data,
                        tmp_file: data?.image,
                    },
            ...
            onUpdateStatus(data, status) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-achievement-summary-status') }}`,
                                method: 'POST',
                                data: {
                                    id: data.id,
                                    status: status
            ...
            onDelete(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-achievement-summary-delete') }}`,
                                method: 'DELETE',
                                data: {
                                    id: data.id
                                }
            ...
            onRestore(data) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-page-achievement-summary-restore') }}`,
                                method: 'PUT',
                                data: {
                                    id: data.id
                                }
