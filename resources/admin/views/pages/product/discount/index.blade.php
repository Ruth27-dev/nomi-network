...
@section('layout')
    <div class="content-wrapper" x-data="productDiscountPage">
        @include('admin::shared.header', [
            'title' => __('form.name.product_discount'),
            'header_name' => __('form.name.product_discount'),
        ])
        <div class="content-body">
            <div class="content-tab">
                ...
                <div class="content-action-button">
                    ...
                    @can('product-discount-create')
                        <button class="btn-create" @click="openStoreProductDiscountDialog()">
            ...
            @include('admin::pages.product.discount.table')
        </div>
        @include('admin::pages.product.discount.store')
    </div>
@stop
@section('script')
    <script type="module">
        Alpine.data('productDiscountPage', () => ({
            table: new Table("{{ route('admin-product-discount-data') }}"),
            init() {
                this.table.init();
                feather.replace();
            },
            ...
            openStoreProductDiscountDialog(id) {
                this.$dialog('storeProductDiscountDialog').open({
                    data: {
                        id: id,
                    },
                    config: {
            ...
            onUpdateStatus(id, status) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-product-discount-status') }}`,
                                method: 'POST',
                                data: {
                                    id: id,
                                    status: status
            ...
            onDelete(id) {
                this.$store.confirmDialog.open({
                    ...
                    afterClosed: (result) => {
                        if (result) {
                            Axios({
                                url: `{{ route('admin-product-discount-delete') }}`,
                                method: 'DELETE',
                                data: {
                                    id: id
                                }
