...
use App\Http\Requests\Admin\Page\WhyChooseUsRequest;
use App\Models\Page;
...
class WhyChooseUsController extends Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->module = [
            'key'   => config('dummy.module.why_choose_us.key'),
        ];
        $this->middleware('permission:why-choose-us-view', ['only' => ['index', 'data']]);
        $this->middleware('permission:why-choose-us-update', ['only' => ['save']]);
    }

    public function index()
    {
        $data['page'] = Page::where('page', 'why_choose_us')->first();
        return view("admin::pages.page.why-choose-us", $data);
    }

    public function save(WhyChooseUsRequest $request)
    {
        DB::beginTransaction();
        try {
            $input = [
