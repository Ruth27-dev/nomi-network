...
use App\Http\Requests\Admin\Page\OurProgramRequest;
use App\Models\Page;
...
class OurProgramController extends Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->module = [
            'key' => config('dummy.module.our_program.key'),
        ];
        $this->middleware('permission:our-program-view', ['only' => ['index', 'data']]);
        $this->middleware('permission:our-program-update', ['only' => ['save']]);
    }

    public function index()
    {
        $data['page'] = Page::where('page', 'our_program')->first();
        return view("admin::pages.page.our-program", $data);
    }

    public function save(OurProgramRequest $request)
    {
        DB::beginTransaction();
        try {
            $input = [
